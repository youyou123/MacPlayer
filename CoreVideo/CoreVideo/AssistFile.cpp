...
int CAssistFile::ParseAssistDataForNovatek(string strMOVFile)
{
    ...
    sprintf(tFile, "%s", strMOVFile.data());
    sprintf(szFile, "%s",tFile);
    //  wsprintf(tFile, %s, strMOVFile);//_T("%s")
    //  sprintf(szFile, "%s", T2A(tFile));
        ...
        while(!feof(fd)) {
            ...
            if(check_flag == 1) {
                ...
                do {
                    ...
                    int check_flag2 = 0;
                    while(ftell(fd) <= pos) {//_ftelli64(fd) <= pos
                        
                        if(fread(chSize, 1, 4, fd) != 4) break;		//get address
                        size = ENDIAN_CONV(chSize);					//convert size
                        if(fread(chFlag, 1, 4, fd) != 4) break;
                        
                        // TRACE("3                [0x%08X]%s - Pos:0x%08X\n", *((unsigned int*)chSize), chFlag, pos);
                        printf("3         [0x%08X]%s - Pos:0x%08X\n", *((unsigned int*)chSize), chFlag, pos);
                    ...
                    while(ftell(fd) <= pos) {
                        ...
                        //  TRACE("4                        [0x%08X]%s - Pos:0x%08X\n", *((unsigned int*)chSize), chFlag, pos);
                        printf("4        [0x%08X]%s - Pos:0x%08X\n", *((unsigned int*)chSize), chFlag, pos);
                    ...
                    while(ftell(fd) <= pos) {
                        ...
                        //   TRACE("5                                [0x%08X]%s - Pos:0x%08X\n", *((unsigned int*)chSize), chFlag, pos);
                        printf(" 5        [0x%08X]%s - Pos:0x%08X\n", *((unsigned int*)chSize), chFlag, pos);
                    ...
                    unsigned int stsz_addr =ftell(fd);   //ftelli64(fd);
                    ...
                    //@liangfu 20160804.
                    {
                        ...
                        //  printf("7	Founded %d stsz indexs! mdat_addr:0x%08X, curr:0x%08X\n", size, mdat_addr, ftell(fd));
                        //    printf(" 6        [0x%08X]%s - Pos:0x%08X\n", *((unsigned int*)chSize), chFlag, pos);
                        ...
                        for(i = 0; i < size; i++) {
                            ...
                            if(i > 0 && (i % 30) == 0) {			//Read video frame size by every 30fps(at the 31frame)
                                ...
                                //     printf("\tSTCO    idx%d: 0x%08X  size:  0x%08X \n", idx, addr, allaframesize[idx - 1]);
                            }
                        }
                    ...
                    if(track_id == 1) {	//only video tracker
                        ...
                        //   TRACE("6	Founded %d stsz indexs! mdat_addr:0x%08X, curr:0x%08X\n", size, mdat_addr, _ftelli64(fd));
                    ...
                    for(i = 0; i < gps_rec_cnt; i++) {
                        ...
                        //_fseeki64(fd, alladdr[i] + 48/*72 Fuck!!!*/, SEEK_SET);
                        //_fseeki64(fd, alladdr[i]+48, SEEK_SET);
                        ...
                        //_fseeki64(fd, alladdr[i] + 72, SEEK_SET);
