...
@property (weak) IBOutlet NSImageView *carIcon;

@property (weak) IBOutlet NSImageView *directionMark;

@property (weak) IBOutlet NSView *speedView;
@end
...
-(void)dataLogicProcessOfViodePath:(NSString *)playVideoPath{//---------------------------new 修改过的xyz，获取偏移数据
...
-(void)updateDataByCurrentTime:(Float64)time{
    ...
    if(currentGsensorDataArr!=nil&&[currentGsensorDataArr count]>0){
        if(index1<[currentGsensorDataArr count]){
            ...
            [_xTextField setStringValue:[NSString stringWithFormat:@"X: %.3f",xVal]];
            [_yTextField setStringValue:[NSString stringWithFormat:@"Y: %.3f",yVal]];
            [_zTextField setStringValue:[NSString stringWithFormat:@"Z: %.3f",zVal]];
