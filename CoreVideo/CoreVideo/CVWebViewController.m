...
- (void)viewDidLoad {
    ...
    NSButton *zoomInOut=[[NSButton alloc] initWithFrame:NSMakeRect(0, 0,30, 30)];
    textField=[[NSTextField alloc] initWithFrame:NSMakeRect(49,5,150 ,22)];
      NSButton *current=[[NSButton alloc] initWithFrame:NSMakeRect(100, 0,90, 30)];
    ...
    //[current setImage:[NSImage imageNamed:@"scale"]];
    [current.cell setImageScaling:NSImageScaleProportionallyDown];
    [current setToolTip:NSLocalizedString(@"zoomin_out",nil)];
    [current setTarget:self];
    [current setTitle:@"BaiduMap"];
    [current setAction:@selector(current:)];
